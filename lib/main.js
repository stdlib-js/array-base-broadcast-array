...
function broadcastArray( x, inShape, outShape ) {
	...
	for ( i = N-1; i >= 0; i-- ) {
		...
		dim = outShape[ i ];
		if ( dim !== 0 && dim < d ) {
			throw new Error( format( '1hN5F', copy( inShape ).join( ', ' ), copy( outShape ).join( ', ' ), i ) );
		}
		if ( d === dim ) {
			...
			throw new Error( format( '1hN5G', copy( inShape ).join( ', ' ), copy( outShape ).join( ', ' ), i ) );
		}
	}
